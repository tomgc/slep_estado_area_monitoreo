# Estructura del proyecto (escaner)

- Raiz       : slep_estado_proyectos_monitoreo
- Fecha      : 2026-06-28 11:08:27
- Directorios: 12
- Archivos   : 43
- Tamano     : 264K

## Arbol

```
slep_estado_proyectos_monitoreo/
00_escanear_proyecto.R  (4.84K)
00_run_all.R  (4.55K)
10_utils/
  10_configuracion.R  (5.19K)
  10_utils.R  (5.19K)
20_insumos/
  registro_proyectos.csv  (1.42K)
30_procesamiento/
  31_descubrir_proyectos.R  (7.06K)
  32_localizar_documentos.R  (7.19K)
  33_extraer_metadatos.R  (3.16K)
  34_compilar_inventario.R  (5.95K)
  35_compilar_panorama.R  (8.17K)
40_salidas/
  cache/
    slep_alertas_ael.md  (1.75K)
    slep_aprendizajes_ep.md  (1.87K)
    slep_categoria_desempeno.md  (1.81K)
    slep_costapresente.md  (1.67K)
    slep_dashboard_personal_monitoreo.md  (1.99K)
    slep_georreferenciacion.md  (1.71K)
    slep_idps.md  (1.78K)
    slep_minuta_asistencia.md  (1.69K)
    slep_minuta_desvinculacion.md  (1.76K)
    slep_rendimiento_historico.md  (1.97K)
    slep_reportes_modelo_resguardo_asistencia.md  (1.83K)
    slep_seguimiento_educacion_inicial.md  (2.01K)
    slep_simce_adecuado.md  (2.03K)
    slep_simce_estandares_aprendizaje.md  (1.92K)
  inventario_cartera.json  (22.2K)
  inventario_cartera.parquet  (17.4K)
  panorama.md  (27.1K)
50_documentacion/
  activa/
    POLITICA_PROYECTO.md  (29.7K)
    SETTINGS_Y_PROMPTS_OPERACIONALES.md  (37.3K)
    decisiones/
      20260628_decision_arquitectura_orquestador.md  (4.21K)
    esbozo_fase2_estado_estandarizado.md  (4.36K)
    reporte_cobertura_documental.md  (6.38K)
  andamios/
  estructura/
    20260628_110608_estructura.md  (2.04K)
    20260628_110608_estructura.txt  (2.04K)
    20260628_110633_estructura.md  (2.11K)
    20260628_110633_estructura.txt  (2.11K)
    estructura_actual.md  (2.11K)
    estructura_actual.txt  (2.11K)
  traspasos/
    traspaso_cierre_v01.md  (13.1K)
CLAUDE.md  (2.42K)
README.md  (4.24K)
slep_estado_proyectos_monitoreo.Rproj  (248)
tests/
  test_orquestador.R  (4.54K)
```

## Conteo por extension

```
  md                 26
  R                  10
  txt                3
  Rproj              1
  csv                1
  json               1
  parquet            1
```
